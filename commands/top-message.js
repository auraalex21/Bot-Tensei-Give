...
export const data = new SlashCommandBuilder()
  .setName("top-message")
  .setDescription("Afficher le classement des utilisateurs par messages");

export async function execute(interaction) {
  const guildId = interaction.guild.id;
  const topUsers = await getTopMessageUsers(guildId);
  ...
  ctx.fillText("🏆 Classement des Messages", canvasWidth / 2, 60);
  ...
  for (let i = 0; i < topUsers.length; i++) {
    ...
    ctx.fillText(`${user.messages} Messages`, canvasWidth - 350, baseY);
  ...
  const attachment = new AttachmentBuilder(canvas.toBuffer(), {
    name: "top-message.png",
  });

  return interaction.reply({
    content: "Voici le classement des utilisateurs par messages :",
    files: [attachment],
    ephemeral: false,
  });
}
